...
TTCthr = 10; % in seconds (TTC Threshold for selection of scenarios)
frameRate = 25; % every 40ms
ScLength = 13; % in seconds (Length of the scenario)
...
% Simulation time for generation of POG
T = 0.04; % time step
totalTime = 3.0; % total prediction horizon

% Sample visualization
sampleVisualization = false;

%% Start reading data from videos
for i = 1:nVideos % 60 tracks available
    ...
    for j = 1 : length(tracks)
        ...
        for k = 1 : length(tracks)
            ...
            scenario(counter1).Target(counter2).bbox = ...
                bboxVecTG(TTCidxTG:TTCidxTG+frameRate*ScLength,:);
            scenario(counter1).Target(counter2).Vx = ...
                VxVecTG(TTCidxTG:TTCidxTG+frameRate*ScLength,:);
            scenario(counter1).Target(counter2).Vy = ...
                VyVecTG(TTCidxTG:TTCidxTG+frameRate*ScLength,:);
            scenario(counter1).Target(counter2).Ax = ...
                AxVecTG(TTCidxTG:TTCidxTG+frameRate*ScLength,:);
            scenario(counter1).Target(counter2).Ay = ...
                AyVecTG(TTCidxTG:TTCidxTG+frameRate*ScLength,:);
        ...
        if counter2~=0
            scenario(counter1).EGO.bbox =...
                bboxVec(TTCidx: TTCidx+(frameRate*ScLength),:);
            scenario(counter1).EGO.Vx = ...
                VxVec(TTCidx: TTCidx+(frameRate*ScLength));
            scenario(counter1).EGO.Vy = ...
                VyVec(TTCidx: TTCidx+(frameRate*ScLength));
            scenario(counter1).EGO.Ax = ...
                AxVec(TTCidx: TTCidx+(frameRate*ScLength));
            scenario(counter1).EGO.Ay = ...
                AyVec(TTCidx: TTCidx+(frameRate*ScLength));
            scenario(counter1).EGO.THW = ...
                THWVec(TTCidx: TTCidx+(frameRate*ScLength));
            scenario(counter1).EGO.TTC = ...
                TTCVec(TTCidx: TTCidx+(frameRate*ScLength));
...
%% Generate data
numScenarios = 50;%size(scenario, 2);
for i = 1:numScenarios
    if i == 1 || rem(i,10) == 0
        disp(['Generation of AOG and POG | in percent: ',...
            num2str(i/numScenarios*100), '%']);
    end
    % Position
    vehPosX = [];
    vehPosY = [];
    ...
    y_Road = scenario(i).Road.y+0.01; % to be visible properly in POG

    % Excluding the inner lanes
    x_Road(2:3,:) = [];
    y_Road(2:3,:) = [];

    % START loop to gather relevant information
    for j = 1:size(scenario(i).Target,2)
        x = scenario(i).Target(j).xCG_New(1);
        y = scenario(i).Target(j).yCG_New(1);
        psi = scenario(i).Target(j).psi_New(1);
        ay = scenario(i).Target(j).ay_New(1);
        l = scenario(i).Target(j).bbox(1,3);
        w = scenario(i).Target(j).bbox(1,4);
        v = scenario(i).Target(j).v_New(1);
        ax = scenario(i).Target(j).ax_New(1);
        if (l > 8)
            type = 2;
        else
            type = 1;
    ...
    EGO.PosX = scenario(i).EGO.xCG_New(1);
    EGO.PosY = scenario(i).EGO.yCG_New(1);
    EGO.Vel = scenario(i).EGO.v_New(1);
    EGO.Psi = scenario(i).EGO.psi_New(1);
    EGO.Ax = scenario(i).EGO.ax_New(1);
    EGO.Ay = scenario(i).EGO.ay_New(1);
    if scenario(i).EGO.bbox(3) > 8
        EGO.Type = 2;
    else
        EGO.Type = 1;
    end
        ...
        for k = 1:9
            ...
            for l = 1:size(maxMinSlip,2)
                ...
                if vehLanePos(j) == 1
                    ...
                    vehicleDatabase(j,k,l).vehicleID = j;
                    if k > 6
                        continue
                    end
    ...
    intervalPOG = 2:N; % Estimating POG every 40ms (same as reference data)
    for n = 1:size(intervalPOG,2)
        probAssign(vehicleDatabase, targetProbabilities,...
            intervalPOG(n), scenario(i));
    end

    % Parameters for evaluation
    timeModelBased(i) = toc;
    vehicleCount(i) = j;
    timeToCollision(i,:) = scenario(i).EGO.TTC(1:75);
    timeHeadWay(i,:) = scenario(i).EGO.THW(1:75);

    %% Generation of AOGs and POGs
    AOG(i,:,:,:) = generateAOG(x_Road, y_Road, vehicleDatabase);
    for l1 = 1:size(vehicleDatabase,1)
        for l2 = 1:size(vehicleDatabase,2)
            for l3 = 1:size(vehicleDatabase,3)
                ...
                vehicleDatabase1(l1,l2,l3).vehicleID =...
                    vehicleDatabase(l1,l2,l3).vehicleID;
    ...
    parfor n = 1:75
        warning('off');
        [generatedPOG(n,:,:), generatedPOGID(n,:,:)] = generatePOG_mex...
            (x_Road, y_Road, vehicleDatabase1, intervalPOG(n));
    end
    POG(i,:,:,:) = generatedPOG;
    POG_ID(i,:,:,:) = generatedPOGID;
    clear vehicleDatabase1 vehicleDatabase targetProbabilities
end
% END loop over number of scenarios

%% Pre processing of extracted data before training
% Scaling of AOG to lie within the interval [0,1]
AOG_Scaled = scaleAOG(AOG);
AOG_Training = single(AOG_Scaled);
POG = single(POG);
POG_ID = single(POG_ID);

% Reshaping to feed in as input to the network
% POG_Training = [];
% for i = 427:size(POG,1)
%     POG_Training = [POG_Training; squeeze(POG(i,:,:,:))];
% end

%% Sample visualization of the data
if sampleVisualization
    scenarioNum = 5;
    for idTimeStep = 1:size(scenario(scenarioNum).EGO.xCG,1)
        % Scenario visualization
        plotScenario = visualizeScenario(scenario, scenarioNum, idTimeStep);

        % Metrics visualization
        plotMetric = visualizeMetrics(scenario, scenarioNum, idTimeStep);

        % AOG visualization
        plotAOG = visualizeAOG(AOG_Scaled, scenarioNum);

        % POG visualization
        plotPOG = visualizePOG(POG, scenarioNum, idTimeStep, scenario);

        % Delete the plots for each time step
        delete(plotScenario);
        delete(plotMetric);
        delete(plotAOG);
        delete(plotPOG);
...
mkdir generatedData_230131
saveDir = [pwd '/generatedData_230131/'];
for i = 1:numScenarios
    scenarioInfo = scenario(i);
    timeModelBasedForScenario = timeModelBased(i);
    vehicleCountForScenario = vehicleCount(i);
    timeToCollisionForScenario = timeToCollision(i,:);
    timeHeadWayForScenario = timeHeadWay(i,:);
    AOGForScenario = AOG_Training(i, :, :, :);
    POGForScenario = squeeze(POG(i, :, :, :));
    POGIDForScenario = squeeze(POG_ID(i, :, :, :));
    save([saveDir num2str(i)], 'AOGForScenario', 'POGForScenario',...
        'POGIDForScenario',...
        'timeModelBasedForScenario', 'vehicleCountForScenario',...
        'timeToCollisionForScenario', 'timeHeadWayForScenario',...
        'scenarioInfo');
    clear AOGForScenario POGForScenario POGIDForScenario
end