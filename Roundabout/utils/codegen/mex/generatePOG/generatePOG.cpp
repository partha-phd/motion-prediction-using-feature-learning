...
static emlrtRSInfo emlrtRSI = { 32,    /* lineNo */
  "generatePOG",                       /* fcnName */
  "C:\\Users\\nadarajan\\Documents\\PredictedOccupancyGrid\\Roundabout\\utils\\generatePOG.m"/* pathName */
};
...
static emlrtBCInfo emlrtBCI = { -1,    /* iFirst */
  ...
  "generatePOG",                       /* fName */
  "C:\\Users\\nadarajan\\Documents\\PredictedOccupancyGrid\\Roundabout\\utils\\generatePOG.m",/* pName */
  0                                    /* checkKind */
};
                 ...
                 predictedOccupancyGrid[25200])
{
  ...
  for (i = 0; i < 140; i++) {
    for (j = 0; j < 180; j++) {
      if (k > 25200) {
        emlrtDynamicBoundsCheckR2012b(25201, 1, 25200, &wb_emlrtBCI, sp);
      }

      xpageoffset = (k - 1) << 2;
      SD->f0.gridNew[xpageoffset] = 0.5 * static_cast<real_T>(j) + 20.0;
      SD->f0.gridNew[xpageoffset + 2] = 0.5 * (static_cast<real_T>(j) + 1.0) +
        20.0;
      SD->f0.gridNew[xpageoffset + 1] = 0.5 * static_cast<real_T>(i) + -35.1;
  ...
  /*  Initialisation */
  memset(&SD->f0.occupancyGrid[0], 0, 252000000U * sizeof(real_T));
  ...
  for (i = 0; i < 25200; i++) {
    ...
    SD->f0.b_occupancyGrid[i] = d;
    if (d > 1.0) {
      SD->f0.b_occupancyGrid[i] = 1.0;
    }
  }

  for (i = 0; i < 140; i++) {
    for (j = 0; j < 180; j++) {
      predictedOccupancyGrid[i + 140 * j] = SD->f0.b_occupancyGrid[j + 180 *
        (139 - i)];
    }
  }
